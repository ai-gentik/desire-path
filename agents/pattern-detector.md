...
7. **(Optional enrichment)** `~/.claude/usage-data/facets/` — if this directory exists, read all JSON files and aggregate: (a) group `friction_detail` strings by recurring theme across sessions; (b) count sessions per `friction_counts` category (`wrong_approach`, `buggy_code`, `stale_context`, etc.); (c) note sessions where `outcome === "not_achieved"` or `claude_helpfulness === "unhelpful"`; (d) join by `session_id` with `sessions.jsonl` entries to correlate friction themes with tool-usage patterns. Use this to raise confidence for CLAUDE.md suggestions where facets friction corroborates tool signals, and to surface CLAUDE.md candidates from recurring `friction_detail` themes that have no existing paved artifact. A pattern from facets alone does not qualify — it must still meet the thresholds in §Minimum thresholds using `sessions.jsonl` as the primary source.
